...
public class Player {
    ...
    public void fix() {
        ...
        if (member != null) {
            if (RBW.guild.getSelfMember().canInteract(member)) {
    ...
    public void updateXP(int addXp, int addClanXP) {
        ...
        for (int i = level.getLevel()+1; i < LevelCache.getLevels().size(); i++) {
            if (xp >= LevelCache.getLevel(i).getNeededXP()) {
                ...
                RBW.getGuild().getTextChannelById(Config.getValue("alerts-channel")).sendMessage("<@" + ID + ">").setEmbeds(embed.build()).queue();
        ...
        if (ClanCache.getClan(this) != null) {
            ...
            for (int i = clan.getLevel().getLevel()+1; i < ClanLevelCache.getClanLevels().size(); i++) {
                if (clan.getXp() >= ClanLevelCache.getLevel(i).getNeededXP()) {
                    ...
                    RBW.getGuild().getTextChannelById(Config.getValue("alerts-channel")).sendMessage("<@" + clan.getLeader().getID() + ">").setEmbeds(embed.build()).queue();
